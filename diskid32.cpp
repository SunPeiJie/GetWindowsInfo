...
char * flipAndCodeBytes1(char * str)
{
...
int ReadPhysicalDriveInNTWithZeroRights(char *sBuff, int nLen)
{
	...
	for (int drive = 1; drive < MAX_IDE_DRIVES; drive++){
		//  Try to get a handle to PhysicalDrive IOCTL, report failure 
		//  and exit if can''t. 
		_stprintf_s(driveName, _T("\\\\.\\PhysicalDrive%d"), drive);
		...
		if (hPhysicalDriveIOCTL != INVALID_HANDLE_VALUE) {
			...
			if (DeviceIoControl(hPhysicalDriveIOCTL, IOCTL_STORAGE_QUERY_PROPERTY, &query, sizeof(query), &buffer, sizeof(buffer), &cbBytesReturned, NULL)){
				...
				strcpy_s(modelNumber, &buffer[descrip->VendorIdOffset]);
				strcpy_s(modelNumber, &buffer[descrip->ProductRevisionOffset]);
				strcpy_s(modelNumber, &buffer[descrip->RawPropertiesLength]);
				if (0 == sBuff[0] &&
...
DWORD GetDriveTypeByBus(TCHAR *drive, WORD *type)
{
...
DWORD GetPhysicalDriveFromPartitionLetter()
{
	...
	_stprintf(sysPath, _T("\\\\.\\PhysicalDrive%d"), 1);
	hDevice = CreateFile(sysPath, // drive to open
		GENERIC_READ | GENERIC_WRITE,    // access to the drive
		FILE_SHARE_READ | FILE_SHARE_WRITE,    //share mode
		NULL,             // default security attributes
		OPEN_EXISTING,    // disposition
...
DWORD GetSystemDiskPhysicalNumber(void)
{
	...
	DWORD diskNumber;
	TCHAR sysPath[MAX_PATH];
	_stprintf(sysPath, _T("\\\\.\\PhysicalDrive%d"), 0);
	...
	diskNumber = GetPhysicalDriveFromPartitionLetter();
	return diskNumber;
}
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	//i = 0;
	//char s[100];
	//GetHardDriveSerialNumber(s, 100);

	//WORD w[100];
	//TCHAR driveName[MAX_PATH];
	//_stprintf(driveName, _T("\\\\.\\PhysicalDrive%d"), 0);
	//GetDriveTypeByBus(driveName, w);
